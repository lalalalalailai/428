...
class ACML:
    ...
    def _estimate_cate(self, X: pd.DataFrame, D_residual: np.ndarray,
        ...
        if risk_indicator is not None:
            ...
            cate_mean = np.mean(cate_init[valid_mask])
            shrinkage_factor = 1.0 / (1.0 + self.lambda_agri * risk_vals[valid_mask])
            cate_shrunk = cate_mean + (cate_init[valid_mask] - cate_mean) * shrinkage_factor
            # CATE收缩正则化: 高风险样本CATE向均值收缩
            # 原理: shrinkage_factor = 1/(1 + λ×Risk_i)
            #   - Risk_i 越大 → shrinkage_factor 越小 → CATE越靠近均值 → 正则化越强
            #   - Risk_i 越小 → shrinkage_factor 越接近1 → CATE几乎不变
            # CATE收缩正则化: 高风险样本CATE向均值收缩
            sample_weights *= np.ones_like(risk_vals[valid_mask])
